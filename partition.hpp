...
std::vector<idx_t> partitionMetis (const Graph &graph, 
    MY_SIZE block_size, real_t tolerance, idx_t options[METIS_NOPTIONS] = NULL) {
  ...
  int r = METIS_PartGraphKway(&nvtxs, &ncon, csr.pointIndices(),
      csr.edgeEndpoints(),
      NULL, NULL, NULL, &nparts, NULL, ubvec, options, &objval,
      partition.data());
  if (r != METIS_OK) {
    throw MetisError{r};
  }
...
std::vector<idx_t> partitionMetisEnh (const Graph &graph,
    MY_SIZE block_size, real_t tolerance, idx_t options[METIS_NOPTIONS] = NULL) {
