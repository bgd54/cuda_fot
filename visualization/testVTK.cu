...
void writeHierarchicalColouringVTK(const std::string &filename,
  ...
  MY_SIZE cell_ind = 0;
  double avg_thread_col = 0;
  std::size_t total_cache_size = 0;
  std::size_t total_num_blocks = 0;
  for (const auto &m : memory.colours) {
    double num_cell_colours = std::accumulate(m.num_cell_colours.begin(),
                                              m.num_cell_colours.end(), 0.0);
    avg_thread_col += num_cell_colours;
    total_cache_size += m.points_to_be_cached.size();
    total_num_blocks += m.num_cell_colours.size();
  ...
  std::cout << "Average number of thread colours: "
            << avg_thread_col / total_num_blocks << "\n"
            << "Average reuse factor: "
            << static_cast<double>(total_cache_size) /
                   (problem.mesh.numCells() *
                    problem.mesh.cell_to_node[0].getDim())
            << "\n";
}
